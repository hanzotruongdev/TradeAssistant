...
void LediaEAAssistantTab::CalculateRisk(void){
   ...
   if(radio_group.Value()==0) {
      // risk percentage
      totalRisk = balance * (StringToDouble(risk_percent_input.Text()) / 100);
      risk_fixed_input.Text(DoubleToString(NormalizeDouble(totalRisk, 3), 3));
   }
   else {
      // risk fixed
      totalRisk = StringToDouble(risk_fixed_input.Text());
      double riskPercent = totalRisk/balance*100;
      risk_percent_input.Text(DoubleToString(NormalizeDouble(riskPercent, 3),3));
   }
  
   // tick value from broker
   double tick_value=MarketInfo(Symbol(),MODE_TICKVALUE);
   //If the digits are 3 or 5 we normalize multiplying by 10
   //if(Digits==3||Digits==5)tick_value=tick_value;
   ...
   if(SLPips>0 && tick_value >0){
      double LotSize=totalRisk/(SLPips*tick_value);
